...
public class GateController : Agent
{
    ...
    void OpenGate()
    {
        ...
        // here will fix if sata that inform if it Taxi car will not open else will open 
        Cars = GameObject.FindWithTag("Cars");
        Debug.Log("cars: "+ Cars);
        if (Cars)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        //    Debug.Log(GetCumulativeReward());
         ...
         if ( other.gameObject.tag == "RandomCars")
        {
           // AddReward(-1.0f);
            //    Debug.Log(GetCumulativeReward());
            //   Debug.Log("AddReward -1.0f");
